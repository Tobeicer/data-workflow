...
2. 验证后**不得立即续跑**；短冷却（2026-08-14 实测：15-30 分钟即可恢复，以页面正常为准；原"≥2 小时"已废止）后先**单请求探测**，通过才可小批恢复（3-5 词/批、≥12s 间隔）。
...
5. `prepare-verification` 是唯一允许的人工过验证入口（用户主动触发）。
