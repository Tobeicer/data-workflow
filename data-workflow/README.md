...
状态：正式目录契约；漫立方 tracked 代码与指南已迁移，其他来源入口见下文
...
漫立方新运行产物进入 `data-workflow/runtime/runs/manlifang/<run_id>/`。现有正式批次与交付的大型本地资产仍保持旧路径，待 Task 4B 在原 checkout 中完成清单核对和同盘移动。
